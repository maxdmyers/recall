...
2. **Extract durable knowledge only.** Keep: conventions, gotchas, decisions with rationale,
   ...
   - Project-specific → `agentic-os/knowledge/projects/<project>/<topic>.md`
   - Generalizes across projects → `agentic-os/knowledge/global/<topic>.md`
...
3. **Detect repeated task patterns.** Compare across today's sessions AND against
   ...
   days), append/update an entry in `agentic-os/inbox/proposals.md`:
   ```
   ## <short pattern name>
